...
class LineBotmedController extends \yii\web\Controller {
    ...
    public function actionCurl($message = null) {
        ...
        if (!$last_thread) {
            ...
            //$photo_line2 = $last_thread_photo->ref . ' ' . Url::to(Yii::getAlias('@web').'/riskmed/'.$last_thread_photo->ref, true);;
            //$message = $thread->risk_summary . ' ' . Url::to('http://localhost/rm/frontend/web/index.php?r=riskmed/view&id=' . $thread->id);
            $photo_line = substr(Gallery::widget(['items' => $last_thread_photo->getThumbnailsline($last_thread_photo->ref, $last_thread_photo->risk_review)]), 43, 100);
            //$message = $thread->risk_summary;
