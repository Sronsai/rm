    ...
    public function beforeSave($insert) {
        ...
        foreach ($this->getTableSchema()->columns as $column) { //loop column ทั้งหมด
            ...
            if (empty($attribute)) { //หากได้ค่าว่าง
                ...
                } else {//ถ้าไม่มีเวลา $date_time[0] = $attribute; $time = '';//เวลาว่าง
                }
        ...
        foreach ($this->attributes as $column_name => $value) {
            if (preg_match('/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $value)) {
                if ($value == '0000-00-00 00:00:00') {
                    $this->setAttribute($column_name, null);
                } else {
                    ...
                    $year = (int) $ymd[0];
                    $year = $year + 543;
                    $result = $ymd[2] . '/' . $ymd[1] . '/' . $year . $date_time[1];
                    $this->setAttribute($column_name, $result);
                }
            }
        }
